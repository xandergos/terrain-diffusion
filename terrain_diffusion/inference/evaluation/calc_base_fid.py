...
def calc_base_fid(model_m, 
    ...
    Args:
        ...
        is_consistency_model: Boolean indicating if the model is a consistency model
        ...
        scheduler_steps (int, optional): Number of steps for scheduler. Defaults to 15 if consistency_model is False, otherwise value is ignored.
    ...
    with torch.no_grad():
        ...
        while samples_generated < num_samples:
            ...
            if not is_consistency_model:
                ...
                for t in [np.arctan(scheduler.sigmas[0] / scheduler.config.sigma_data), np.arctan(0.78 / scheduler.config.sigma_data)]:
                    t = torch.tensor([t], device=batch['image'].device).view(1, 1, 1, 1).expand(batch['image'].shape[0], 1, 1, 1)
                    z = torch.randn_like(batch['image']) * scheduler.config.sigma_data
                    x_t = torch.cos(t) * samples + torch.sin(t) * z
                    
                    if cond_img is not None:
                        model_input = torch.cat([x_t / scheduler.config.sigma_data, cond_img], dim=1)
                    else:
                        model_input = x_t / scheduler.config.sigma_data
                    with torch.autocast(device_type="cuda", dtype=dtype):
                        pred = -model_m(model_input, noise_labels=t.flatten(), conditional_inputs=cond_inputs)
                    
                    samples = torch.cos(t) * x_t - torch.sin(t) * scheduler.config.sigma_data * pred
                
            # Only evaluate latents
            latents_std = dataloader.dataset.base_dataset.latents_std.to(samples.device)
            latents_mean = dataloader.dataset.base_dataset.latents_mean.to(samples.device)
            sigma_data = scheduler.config.sigma_data
            latent = (samples[:, :4] / latents_std + latents_mean) / sigma_data
            with torch.autocast(device_type="cuda", dtype=dtype):
                highfreq = autoencoder.decode(latent)[:, :1]
            highfreq = dataloader.dataset.base_dataset.denormalize_residual(highfreq, dataloader.dataset.base_dataset.subset_resolutions[0])
            lowfreq = dataloader.dataset.base_dataset.denormalize_lowfreq(batch['image'][:, 4:5])
            highfreq, lowfreq = laplacian_denoise(highfreq, lowfreq, sigma=5)
            terrain = laplacian_decode(highfreq, lowfreq)

            # Process and update FID metrics
            real_latent = (batch['image'][:, :4] / latents_std + latents_mean) / sigma_data
            with torch.autocast(device_type="cuda", dtype=dtype):
                real_highfreq = autoencoder.decode(real_latent)[:, :1]
            real_highfreq = dataloader.dataset.base_dataset.denormalize_residual(real_highfreq, dataloader.dataset.base_dataset.subset_resolutions[0])
            real_lowfreq = dataloader.dataset.base_dataset.denormalize_lowfreq(batch['image'][:, 4:5])
            real_terrain = laplacian_decode(real_highfreq, real_lowfreq)
            
            # Normalize generated terrain
            ...
            # Normalize real terrain
            real_min = torch.amin(real_terrain, dim=(1, 2, 3), keepdim=True)
            real_max = torch.amax(real_terrain, dim=(1, 2, 3), keepdim=True)
            real_range = torch.maximum(real_max - real_min, torch.tensor(1.0))
            real_mid = (real_min + real_max) / 2
            
            # Process samples
            samples_norm = torch.clamp(((terrain - terrain_mid) / terrain_range + 0.5) * 255, 0, 255)
            samples_norm = samples_norm.repeat(1, 3, 1, 1)
            samples_norm = samples_norm.to(torch.uint8)
            
            real_norm = torch.clamp(((real_terrain - real_mid) / real_range + 0.5) * 255, 0, 255)
...
@click.option("--autoencoder-path", type=click.Path(exists=True), required=True, help="Path to autoencoder model (default: required)")
@click.option("--is-consistency-model", is_flag=True, default=False, help="Is consistency model (default: False)")
...
def calc_base_fid_cli(main_config, 
                        guide_config, 
                        main_sigma_rel, 
                        guide_sigma_rel, 
                        guide_ema_step, 
    ...
    model_m, model_g = create_models(main_resolved, 
                                     ...
                                     guide_ema_step, 
                                     save_models,
                                     is_consistency_model=is_consistency_model)
