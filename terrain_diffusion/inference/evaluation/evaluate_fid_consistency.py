...
from terrain_diffusion.inference.evaluation.utils import create_models, create_models_consistency, get_dataloader
...
def evaluate_models_fid_consistency(model, 
                                    dataloader, 
    ...
    timesteps = torch.as_tensor([np.arctan(80/0.5), *intermediate_timesteps], dtype=torch.float32, device=model.device)
    ...
    with torch.no_grad():
        ...
        while samples_generated < num_samples:
            ...
            cond_inputs = [torch.zeros(batch['image'].shape[0], dtype=torch.int64, device=model.device)]
            ...
            for t in timesteps:
                ...
                with torch.autocast(device_type="cuda", dtype=dtype):
                    pred = -model(model_input, noise_labels=t.flatten(), conditional_inputs=cond_inputs)
...
def evaluate_consistency_fid_cli(config,
    ...
    # Run evaluation
    evaluate_models_fid_consistency(
        model,
        dataloader,
        timestep,
        scale_timesteps,
