...
class EDMDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        scaling_p: float = None,
        scaling_t: float = 0.05,
    ...
    def _compute_karras_sigmas(self, ramp, sigma_min=None, sigma_max=None) -> torch.Tensor:
        ...
        if self.config.scaling_p is not None:
            u = (sigmas - self.config.sigma_min) / (self.config.sigma_max - self.config.sigma_min)
            base_scale = torch.sqrt(torch.maximum(torch.zeros_like(u), 1 - u**self.config.scaling_p))
            sigmas = sigmas / (base_scale * (1 - self.config.scaling_t) + self.config.scaling_t)
        return sigmas
