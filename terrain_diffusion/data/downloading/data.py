...
def initialize_ee():
    ...
    ee.Authenticate()
    ee.Initialize(
        project="recipebase-449613",
        opt_url="https://earthengine-highvolume.googleapis.com",
    )


def download_cell_data(
    image,
    ...
    # Save to local file
    filename = (
        f"{image_name}_{cell_index}.tif" if image_name else f"cell_{cell_index}.tif"
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            params = {
                ...
                "filePerBand": False,
                "maxPixels": 1e9,
                "formatOptions": {"cloudOptimized": False, "compression": "LZW"},
            }
            ...
            if attempt < max_retries - 1:
                ...
                print(
                    f"Error downloading cell {cell_index} after {max_retries} attempts: {e}"
                )
                return None


def calculate_land_percentage(
    cell: Tuple[float, float, float, float], resolution: float = 0.1
) -> float:
    """
...
@click.option(
    "--image",
    type=str,
    help='Image to export. Options: "dem", "copernicus", "landcover_class", "landcover_water", "jrc"',
)
@click.option(
    "--output_dir",
    type=str,
    default="terrain_data",
    help="Directory where the exported data will be saved",
)
@click.option(
    "--output_size", type=int, default=4096, help="Output size of the image in pixels"
)
@click.option(
    "--output_resolution",
    type=float,
    default=90,
    help="Output resolution of the image in meters",
)
@click.option(
    "--land_threshold",
    type=float,
    default=0.1,
    help="Required land coverage percentage per export cell (Default 0.1%)",
)
@click.option(
    "--num_workers",
    type=int,
    default=0,
    help="Number of parallel download workers (0 for sequential)",
)
def download_data_cli(
    image, output_dir, output_size, output_resolution, land_threshold, num_workers
):
    """
    ...
    # Get grid cells
    grid_cells = create_equal_area_grid(
        (output_size * output_resolution, output_size * output_resolution)
    )
    ...
    if image == "dem":
        ...
        # JRC Global Surface Water occurrence band (1984-2021) at native 30m resolution.
        # Primary water label source per the water-mask design: permanent water ≥ 50% occurrence.
        export_image = ee.Image("JRC/GSW1_4/GlobalSurfaceWater").select("occurrence")
        image_name = "jrc"
        native_scale = 30  # native Landsat-derived 30m grid
    else:
        raise ValueError(
            f"Invalid image option: {image}. Please choose 'dem', 'copernicus', 'landcover_class', 'landcover_water', or 'jrc'."
        )
    ...
    if num_workers > 0:
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for future in tqdm(
                as_completed(futures), total=len(futures), desc="Downloading cells"
            ):
        ...
        for i, cell in tqdm(filtered_cells, desc="Downloading cells"):
            filepath = download_cell_data(
                export_image, cell, output_dir, i, image_name, scale=native_scale
            )
            if filepath:
                downloaded_files.append(filepath)
