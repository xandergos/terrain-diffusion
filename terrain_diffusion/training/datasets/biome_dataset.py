...
class BiomeDataset(Dataset):
    def __init__(self,
                 ...
                 *,
                 ...
                 lat_min: float = -60.0,
                 lat_max: float = 60.0):
        # Load rasters in the latitude band [lat_min, lat_max]
        ...
        with rasterio.open(elev_file) as src:
            ...
            elev_img = src.read(1)[start_row:end_row, :]
        ...
        n_rows, n_cols = elev_img.shape
        rng = np.random.default_rng(seed)
        x = np.empty((int(length), 5), dtype=np.float32)
        ...
        with tqdm(total=int(length), desc="Sampling dataset") as pbar:
            while i < int(length):
                ...
                k = koppen_img[r, c]
                if np.isnan(e) or np.isnan(t) or np.isnan(ts) or np.isnan(p) or np.isnan(ps) or np.isnan(k):
                    continue
                x[i, 0] = e
                x[i, 1] = t
                x[i, 2] = ts
