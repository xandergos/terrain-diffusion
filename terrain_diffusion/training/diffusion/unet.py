...
class DiffusionAutoencoder(ModelMixin, ConfigMixin):
    """
    A diffusion model with an encoder for dimensionality reduction.
    
    Args:
        diff_model: The main diffusion model
        encoder: The encoder model for dimensionality reduction
        freeze_encoder: Whether to freeze the encoder parameters
        freeze_diff_model: Whether to freeze the diffusion model parameters
    """
    def __init__(self, diff_model, encoder, freeze_encoder=False, freeze_diff_model=False):
        super().__init__()
        self.diff_model = diff_model
        self.encoder = encoder
        self.register_to_config(diff_model_cfg=diff_model.config, encoder_cfg=encoder.config)
        self.freeze_encoder = freeze_encoder
        self.freeze_diff_model = freeze_diff_model
        
        if freeze_encoder:
            for param in self.encoder.parameters():
                param.requires_grad = False
        if freeze_diff_model:
            for param in self.diff_model.parameters():
                param.requires_grad = False
                
    def preencode(self, x, conditional_inputs=None):
        return self.encoder.preencode(x, conditional_inputs)
    
    def postencode(self, means, logvars, use_mode=False):
        return self.encoder.postencode(means, logvars, use_mode)
    
    def init_decode(self, z):
        size = self.diff_model.config.image_size
        return torch.nn.functional.interpolate(z, size=(size, size), mode='nearest')
    
    def forward_diffusion(self, noisy_x, decoded_x, noise_labels, conditional_inputs, return_logvar=False):
        x = torch.cat([noisy_x, decoded_x], dim=1)
        return self.diff_model(x, noise_labels, conditional_inputs, return_logvar=return_logvar)

    def forward(self, noisy_x, clean_x, noise_labels, conditional_inputs, use_mode=False):
        means, logvars = self.preencode(clean_x, conditional_inputs)
        z = self.postencode(means, logvars, use_mode)
        decoded_x = self.init_decode(z)
        return self.forward_diffusion(noisy_x, decoded_x, noise_labels, conditional_inputs)

    @classmethod
    def from_config(cls, config):
        diff_model_cfg = config['diff_model_cfg']
        encoder_cfg = config['encoder_cfg']
        return cls(
            diff_model=EDMUnet2D.from_config(diff_model_cfg),
            encoder=EDMUnet2D.from_config(encoder_cfg)
        )

    def count_parameters(self):
        return self.diff_model.count_parameters() + self.encoder.count_parameters()
    
class PatchGANDiscriminator(nn.Module):
    """
    A PatchGAN discriminator for image discrimination.
    ...
    def __init__(self, in_channels=1, ndf=64, n_layers=3):
        ...
        for n in range(1, n_layers):
            ...
            sequence += [
                ...
                nn.BatchNorm2d(ndf * nf_mult, momentum=1.0),
                nn.LeakyReLU(0.2, True)
            ]
