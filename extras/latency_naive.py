"""
Measure generation latency (TTFT and TTST) for the naive pipeline.
...
@click.option('--cpu', is_flag=True, default=False, help='Use CPU instead of GPU')
...
@click.option('--decoder-stride', default=512, type=int, help='Decoder tile stride')
def main(onestep_latent, cpu, tile_size, grid_aligned, num_runs, decoder_tile_size, decoder_stride):
    if cpu:
        ...
        if device == 'cpu':
            print("Note: CUDA not available, falling back to CPU")
